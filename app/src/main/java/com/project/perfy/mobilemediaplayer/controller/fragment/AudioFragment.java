...
public class AudioFragment extends BaseFragment {
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (mediaItems != null && mediaItems.size() > 0) {
                // 有数据
                // 设置适配器
                videoFragmentAdapter = new VideoFragmentAdapter(mContext, mediaItems,false);
    ...
    private void getDataFromLocal() {
        ...
        new Thread() {
            ...
            public void run() {
                super.run();
                // 获取音频列表的时候需要过滤音频类型，否则在序列化intent传值时会崩溃。
                String where =  "mime_type in ('audio/mpeg','audio/x-wav','audio/x-ms-wma' and is_music > 0)" ;
                String sortOrder = MediaStore.Audio.Media.DATA;
                isGrantExternalRW((Activity) mContext);
                //SystemClock.sleep(2000);
                Uri uri = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI;
                String[] objs = {
                        MediaStore.Audio.Media.DISPLAY_NAME, // 视频文件在sdcard的中的名称
                        MediaStore.Audio.Media.DURATION, // 视频的总时长
                        MediaStore.Audio.Media.SIZE, // 视频文件的大小
                        MediaStore.Audio.Media.DATA, // 视频的绝对地址
                        MediaStore.Audio.Media.ARTIST // 演唱者（歌曲文件的属性）
                };
                ContentResolver resolver = mContext.getContentResolver();
                //Cursor cursor = resolver.query(uri, objs, null, null, null);
                Cursor cursor = resolver.query(uri, objs, where, null, sortOrder);
                if (cursor != null) {
