...
/**
 * Activity和service交互的方式：广播、Intent、Handler、接口回调、Application、EventBus、AIDL
 */

/**
 * EventBus3.0
 * 1.方法名，不需要onEvent开头
 * 2.方法的线程模式ThreadMode可以配置，两个方法通用一个参数，可以设置优先级priority，值越大，优先收到。
 * 使用EventBus3.0
 * 1.关联
 * 2.注册
 * 3.订阅方法
...
public class SystemAudioPlayer extends Activity implements View.OnClickListener {
     ...
     * <br />
     * Auto-created on 2017-04-08 14:44:16 by Android Layout Finder
    ...
    private void showPlayMode() {
        try {
            ...
            if(playMode == MusicPlayerService.REPEAT_NORMAL) {
                ...
                Toast.makeText(SystemAudioPlayer.this, "单曲循环", Toast.LENGTH_SHORT).show();
            }else if(playMode == MusicPlayerService.REPEAT_ALL){
                btnAudioPlaymode.setBackgroundResource(R.drawable.btn_audio_playmode_all_selector);
                Toast.makeText(SystemAudioPlayer.this, "列表循环", Toast.LENGTH_SHORT).show();
            }else{
                btnAudioPlaymode.setBackgroundResource(R.drawable.btn_audio_playmode_normal_selector);
                Toast.makeText(SystemAudioPlayer.this, "顺序播放", Toast.LENGTH_SHORT).show();
            }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case SHOW_LYRIC: // 显示歌词
                    ...
                    try {
                        // 1.得到当前的进度
                        int currentPosition = service.getCuttentPosition();
                        // 2.把进度传入ShowLyricView控件，并且计算该高亮显示哪一句
...
//        registerReceiver(receiver, intentFilter);
    ...
    private void bindAndStartService() {
        ...
        intent.setAction("com.porfirio.mobileplayer_OPENAUDIO");

        Bundle bundle = new Bundle();
        bundle.putSerializable("audiolist", (Serializable) mediaItems);
        intent.putExtras(bundle);
        intent.putExtra("position", position);

        bindService(intent, conn, Context.BIND_AUTO_CREATE);
        startService(intent); // 不会因为多次执行，而实例化多个服务。
    }
