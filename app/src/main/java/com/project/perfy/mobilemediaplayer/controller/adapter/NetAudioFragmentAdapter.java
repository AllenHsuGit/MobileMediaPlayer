...
public class NetAudioFragmentAdapter extends BaseAdapter {
    ...
    private List<NetAudioPagerBean.ListBean> mDatas;
    private Utils utils;

    public NetAudioFragmentAdapter(Context context, List<NetAudioPagerBean.ListBean> datas) {
    ...
    public int getItemViewType(int position) {
        NetAudioPagerBean.ListBean listBean= mDatas.get(position);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //绑定数据
        //根据位置得到数据,绑定数据
        NetAudioPagerBean.ListBean mediaItem = mDatas.get(position);
        bindData(itemViewType, viewHolder, mediaItem);

        return  convertView;
    }

    private void bindData(int itemViewType, ViewHolder viewHolder, NetAudioPagerBean.ListBean mediaItem) {
        switch (itemViewType) {
            case TYPE_VIDEO://视频
...
//                    x.image().bind(viewHolder.iv_image_icon, mediaItem.getImage().getBig().get(0));
    ...
    private void bindData(ViewHolder viewHolder, NetAudioPagerBean.ListBean mediaItem) {
        ...
        //设置标签
        List<NetAudioPagerBean.ListBean.TagsBean> tagsEntities = mediaItem.getTags();
